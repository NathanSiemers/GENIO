...
source("one.set.vars.R", echo = TRUE)

##################################################################
## This R/Markdown code/document, when knitted/executed, will:
##     1. Download TCGA genomic data from USCS Pan-Cancer
##     2. Build and save merged clinical-rna-mutation and clinical-rna-cnv data
##     3. Perform analysis of the above data sets
##     4. Generated data structures ok for reporting and other ones good for analytics
##     5. Generate manuscript, tables, figures
################################################################
## see the top of one.set.vars.R for important flags to set before running
##
## you will need a hefty machine with about 50 GB of RAM per CPU used
##
## on an 80 CPU linux server with gobs of RAM the entire analysis completes
## in approximately 12 hours, not including file download time
##
## the CRAN and Bioconductor R packages you need should be easy
## to identify there also (around line 190 of one.set.vars.R)
##
## snapshot of TCGA data used can be found at http://fiveprime.org/GENIO
################################################################
## my code started out pretty. it is not pretty now.
## it is released to aid in reproducibility
## Nathan Siemers
## portions of code contributed by Han Chang and Jim Holloway
################################################################


```

```{r two_main_compute, eval = compute.eval }

source("two.main.compute.R", echo = TRUE)
...
`r length(unique(o.g.sig$Cohort))`
...
[@Tirosh2016](Fig `r netbox = plus('figure.number'); netbox`A).

```{r , fig.width = 7.5, fig.height = 7.5, fig.keep = 'last', fig.show = 'hide', dev='tiff', dev.args = list(compression = 'lzw'), dpi = 600}

library(tiff)

##pdf(file = 'gridtest.pdf', width = 7, height = 7)

grid.arrange(
grobTree(grid::rasterGrob(readTIFF('tirosh-cd4.tiff')), textGrob("A", x=0.02,  y=0.95, hjust=0, gp=gpar(col="black", fontsize=25) ) ),
grobTree(grid::rasterGrob(readTIFF('foxp3.tcga.tiff')) ,textGrob("B", x=0.02,  y=0.95, hjust=0, gp=gpar(col="black", fontsize=25) ) ),
grobTree(grid::rasterGrob(readTIFF('foxp3.ssmel.tiff')) ,textGrob("C", x=0.02,  y=0.95, hjust=0, gp=gpar(col="black", fontsize=25) ) ),
grobTree(grid::rasterGrob(readTIFF('mfm2.tcga.tiff')) , textGrob("D", x=0.02,  y=0.95, hjust=0, gp=gpar(col="black", fontsize=25) ) ),
ncol = 2  )
...
**Figure `r plus('figure.number.printed')`. Expression and correlation of immunological markers in TCGA and Tirosh et al. single cell melanoma RNA-seq data.**
...
`r round(cor(rnacnv$FOXP3, rnacnv$CTLA4), digits = 2)`)
and two markers
with correlation below 0.1 (IL4,
`r round(cor(rnacnv$FOXP3,rnacnv$IL4), digits = 2)`; IL5,
`r round(cor(rnacnv$FOXP3,rnacnv$IL5), digits = 2)`).





```{r gitrcor, eval = FALSE}

Other possible candidate markers for Treg cells include
TIGIT, PTPRC (CD25), and TNFRSF18 (GITR).  TIGIT exhibited high
correlation to FOXP3
(`r round(cor(rnacnv$FOXP3, rnacnv$TIGIT), digits= 2)`).
However, TIGIT's correlation to CD8A (a marker for CD8+ T
cells) was higher
(`r round(cor(rnacnv$CD8A, rnacnv$TIGIT), digits =2)`).
CD25 and GITR correlations to FOXP3 were modest
(FOXP3-CD25, correlation `r round(cor(rnacnv$FOXP3, rnacnv$PTPRC), digits = 2)`;
FOXP3-GITR, correlation `r round(cor(rnacnv$FOXP3,rnacnv$TNFRSF18), digits = 2)`).

Finally, TNFRSF18 (GITR) had a higher correlation to FOXP3
(`r round(cor(rnacnv$FOXP3, rnacnv$TNFRSF18), digits = 2)`)
than CD8A
(`r round(cor(rnacnv$CD8A, rnacnv$TNFRSF18), digits = 2)`),
but the correlation to FOXP3 was not high.
)
and
Despite
their reasonable correlation to FOXP3, both were more highly correlated
to CD8+ T cell markers like CD8A (CD8A-PTPRC correlation
`r round(cor(rnacnv$CD8A, rnacnv$PTPRC), digits = 2)`;
CD8A-TIGIT correlation
...
Figure `r netbox`B presents one example, a view of the (mutual rank) co-regulatory
...
the co-expression of FOXP3 and CCR8 (Figure `r netbox`C).
FOXP3 and CCR8 were
ultimately chosen as a signature set for Treg estimation. Signature sets for other immune cell types were derived
in a similar fashion; for macrophages, a highly co-regulated set (VSIG4, CD163, MS4A4A) was derived (Macrophage: Figure `r netbox`D).
...
```{r phantom_treg, results='asis', eval=FALSE}

## not used any more eval=FALSE

  ## phantom.treg = c('Sample_Type', 'Cohort', 'TGFB1', 'ENTPD1', 'IL2RA', 'IL10', 'IL32', 'FOXP3', 'CTLA4', 'CCR4',
  ##       'GPR15', 'IKZF2')

phantom.treg = c('FOXP3','C15orf53','IL5','CTLA4','IL32', 'GPR15', 'IL4' )

  p.rna = rnacnv[, which(  colnames(rnacnv) %in% c(phantom.treg, 'Cohort')  ) ]
  ## tumor only
  ##p.rna = p.rna[! grepl('ormal', p.rna$Sample_Type), ]
  ind = sapply(p.rna, is.numeric)
  p.rna = ddply(p.rna, .(Cohort), function(x) {
        x[ , ind] = scale(x[ , ind])
         x})
  my.cor = cor(p.rna[, ind], p.rna[, ind])[ , "FOXP3", drop=FALSE]
  my.cor = my.cor[order(my.cor[, "FOXP3"], decreasing=TRUE), , drop=FALSE]
  colnames(my.cor)[1] = "Correlation with FOXP3"
  my.cor = my.cor[rownames(my.cor) != 'FOXP3', , drop = FALSE]


kable(my.cor, digits=2)

```


**Table `r plus('table.number.printed')`. RNA-seq based marker sets created and used in this study to estimate immune cell levels in tumors.**

```{r, sig_table, results='asis'}

  dlist =  (decon_signature_list)
  dlist = dlist[ names(dlist) %in% paper.sigs ]
  dlist$TregCD8 = c('Treg', 'TCD8')
  dlist$NKCD8 = c('NK', 'TCD8')

  desc = list(
        TCD8.sig = "CD8+ T cell",
        Treg.sig = "Regulatory T cell",
        Tcell.sig = "T cell (general)",
        Bcell.sig = "B cell",
        NK.sig = "Natural killer cell",
        ##MGran.sig = "Myeloid granulocyte",
        Mono.sig  = "Monocyte",
        MFm2.sig = "M2 Macrophage",
        TregCD8 = "Treg versus CD8+ T cell",
        NKCD8 = "NK versus CD8+ T cell"
        )
  table = ldply(names(dlist) , function(sig) {
        data.frame(
              Signature = gsub(".sig", "", sig),
              Membership = paste(dlist[[sig]], collapse = ", "),
...
modulator of the nuclear factor kappa-B pathway  (Fig `r plus('figure.number')`).
...
box1 = plus('figure.number')

```

```{r , fig.width = 7.5, fig.height = 5.62, fig.keep = 'last', fig.show = 'hide', dev='tiff', dev.args = list(compression = 'lzw'), dpi = 600}

cd8hn = o.g[o.g$Cohort == 'Head_and_Neck_Cancer' & o.g$Sig == 'TCD8.sig', ]
dim(cd8hn)
manhat4(cd8hn, scale.legend = TRUE, p.thresh = 0, mut.p.thresh = 0.5)

pdf(file = "manhat4.pdf", width = 7.5, height = 5.62)
manhat4(cd8hn, scale.legend = TRUE, p.thresh = 0, mut.p.thresh = 0.5)
dev.off()
...
##display.P = 1
#manhat2( o.g.sig[which (abs(o.g.sig$P) > display.P , ), ], minSize = 0.1, maxSize = 2)


```

**Figure `r fig.landscape = plus('figure.number.printed'); fig.landscape`. Landscape of association between tumor copy number changes, mutations,  and CD8+ T cell estimates in  TCGA head and neck cancer.**
...
linked to loss of the chromosome 9p region (Fig `r box1`A-B).
...
a_grob = grobTree(textGrob("A", x=0.05,  y=0.95, hjust=0,
    gp=gpar(col="black", fontsize=25) ) )
b_grob = grobTree(textGrob("B", x=0.05,  y=0.95, hjust=0,
    gp=gpar(col="black", fontsize=25) ) )
c_grob = grobTree(textGrob("C", x=0.05,  y=0.95, hjust=0,
    gp=gpar(col="black", fontsize=25) ) )
d_grob = grobTree(textGrob("D", x=0.05,  y=0.95, hjust=0,
    gp=gpar(col="black", fontsize=25) ) )



##pdf(file = 'foo.pdf', width = 7.5, height = 7.5)
grid.arrange(
    myplot('CDKN2A.cnv', 'Bcell.sig', 'Melanoma', rnacnv) + annotation_custom(a_grob),
    myplot('CDKN2A.cnv', 'Tcell.sig', 'Melanoma', rnacnv) + annotation_custom(b_grob),
    mybox('TP53.mut', 'Treg.sig', 'Breast_Cancer', rnamut) + annotation_custom(c_grob),
    mybox('TP53.mut', 'TCD8.sig', 'Head_and_Neck_Cancer', rnamut) + annotation_custom(d_grob),
    ncol = 2  )
##dev.off()

```

**Figure `r plus('figure.number.printed')`. Association of CDKN2A CNA and TP53 mutation with immune estimates in tumors.**
(A-B)
...
```{r , fig.width = 7.5, fig.height = 7.5, fig.keep = 'last', fig.show = 'hide', dev='tiff', dev.args = list(compression = 'lzw'), dpi = 600}
  ...
  manhat2(o.g.sig[o.g.sig$Cohort == 'Melanoma' & o.g.sig$Chr == 9, ], p.view.threshold=0,
          labels = c( 'CDKN2A.cnv',  "JAK2.cnv" ), rotate = 90, lSize = 3,
          size=5, vjust=0.5, hjust= -0.2, minSize = 0.5, maxSize = 4 )

```

**Table `r plus('table.number.printed')`. Associations of loss of CDKN2A with estimates of immune cell types in tumors.**

```{r kablecdkn2a, results='asis'}

df = hit.kable2(c('CDKN2A.l'), df.only = TRUE, coords = FALSE)
df$Adrenocortical = NULL
colnames(df) =
 gsub('carcinoma| Cell|and ', '', colnames(df) )
##df$Variant = "9p loss"
df$Variant = NULL
kable(df)

##    Data are shown for loci where -logP > `r display.P`  and effect size is indicated if -logP >
## `r effect.cutoff.P`.
...
**Figure `r plus('figure.number.printed')`. Relationship between chromosome 9 genetic changes and immune cell abundance estimates in TCGA melanoma.**
...
`r plus('table.number')`,
Fig `r box1`C).   However, TP53 mutation was
...
(-logP = `r o.g.sig[o.g.sig$Variant == 'SYCP2.mut' & o.g.sig$Cohort == 'Head_and_Neck_Cancer' & o.g.sig$Sig == 'Treg.sig', "P"]`,
effect size
`r o.g.sig[o.g.sig$Variant == 'SYCP2.mut' & o.g.sig$Cohort == 'Head_and_Neck_Cancer' & o.g.sig$Sig == 'Treg.sig', "Effect"]`)
and the Treg/CD8 T cell ratio in head and neck cancer
(Fig `r box2 = plus("figure.number"); box2`A.)[@Yang2006].


**Table `r plus('table.number.printed')`. Associations of TP53 mutations with estimates of immune cell levels across TCGA cohorts.**

```{r p53table, results='asis'}

##  my.list =     c('TP53.mut')
##  hit.kable2(my.list, coords = FALSE)

out = o.g.sig[ grepl('TP53\\.', o.g.sig$Variant) & o.g.sig$P > 1.3 , ]
out = out[! grepl('\\.[gl]$', out$Variant), ]

out = data.frame(
    Cohort = out$Cohort,
    Sig = gsub('\\.sig$', '', out$Sig),
    Variant = gsub('.mut', ' mutant', out$Variant),
    Association = paste0(out$Effect, ' (', out$P, ')')
    )
...
##pdf(file = 'foo.pdf', width = 7.5, height = 7.5)
grid.arrange(
    mybox('SYCP2.mut', 'TregCD8.sig', 'Head_and_Neck_Cancer', rnamut) + annotation_custom(a_grob),
    #mybox('ARID2.mut', 'TCD8.sig', 'Colon_Cancer', rnamut) + annotation_custom(b_grob),
    myplot('CDKN2A.cnv', 'FZD3', 'Melanoma', rnacnv) + annotation_custom(b_grob),
    mybox('FGFR3.mut', 'MFm2.sig', 'Bladder_Cancer', rnamut) + annotation_custom(c_grob),
...
**Figure `r plus('figure.number.printed')`. Association of SYCP2 and FGFR3 with immune estimates in tumors; correlation of chromosome 9p copy number (CDKN2A) with FZD3 RNA expression.**
...
`r round(o.g.sig[o.g.sig$Cohort == 'Melanoma' & o.g.sig$Variant == 'B2M.mut' & o.g.sig$Sig == 'NK.sig', "Effect"], digits = 1)`, -logP =
`r round(o.g.sig[o.g.sig$Cohort == 'Melanoma' & o.g.sig$Variant == 'B2M.mut' & o.g.sig$Sig == 'NK.sig', "P"], digits = 1)`) and lung
adenocarcinoma (effect size
`r round(o.g.sig[o.g.sig$Cohort == 'Lung_Adenocarcinoma' & o.g.sig$Variant == 'B2M.mut' & o.g.sig$Sig == 'NK.sig', "Effect"], digits = 1)`, -logP
`r round(o.g.sig[o.g.sig$Cohort == 'Lung_Adenocarcinoma' & o.g.sig$Variant == 'B2M.mut' & o.g.sig$Sig == 'NK.sig', "P"], digits = 1)`
...
`r round(o.g.sig[o.g.sig$Cohort == 'COAD_MSS' & o.g.sig$Variant == 'JAK2.mut' & o.g.sig$Sig == 'NK.sig', "Effect"], digits = 1)`,
                        -logP =
`r round(o.g.sig[o.g.sig$Cohort == 'COAD_MSS' & o.g.sig$Variant == 'JAK2.mut' & o.g.sig$Sig == 'NK.sig', "P"], digits = 1)`).



```{r corpestetc }

cor.pest <<- round(cor(rnacnv[rnacnv$Cohort == 'Melanoma' & rnacnv$MYC.cnv > -0.1, "MYC.cnv"], rnacnv[rnacnv$Cohort == 'Melanoma' & rnacnv$MYC.cnv > -0.1, "PEst"]  ), digits = 2)

cor.tcd8 <<- round(cor(rnacnv[rnacnv$Cohort == 'Melanoma' & rnacnv$MYC.cnv > -0.1, "MYC.cnv"], rnacnv[rnacnv$Cohort == 'Melanoma' & rnacnv$MYC.cnv > -0.1, "TCD8.sig"]  ), digits = 2)
...
(Table `r table.apc = plus('table.number'); table.apc`).
Although one can observe negative correlations between MYC copy number gains and
CD8 T cell estimates in melanoma (Pearson correlation: `r cor.tcd8`) and some other tumor types,
the MYC copy number levels are also highly correlated with purity estimates (melanoma
Pearson correlation: `r cor.pest`). We did not observe a significant association of MYC copy number
...
in head/neck, stomach, and the Luminal B subtype (LumB) of breast cancer  (Table `r table.apc`).
Finally, we noticed a strong correlation between WNT receptor Frizzled 3 (FZD3) expression and 9p
loss in melanoma (Fig `r box2`C.)


**Table `r plus('table.number.printed')`. Associations of APC mutation
and MYC copy number gains with estimates of immune cell levels.**


```{r 9ptable, results='asis'}

hit.kable2(c('APC.mut', 'MYC.g'), coords = FALSE)
...
`r plus("table.number")`). Despite the high frequency of
...
**Table `r plus('table.number.printed')`. Association of RAS/RAF oncogene family member mutations with estimates of immune cell types across TCGA.**

```{r rastable, results='asis'}

  my.list = c( 'NRAS.mut', 'HRAS.mut', 'KRAS.mut', 'BRAF.mut' )
...
tumor types (Table `r plus("table.number")`). We observed association with both
...
in head and neck cancer (Table `r plus("table.number")` and Fig `r box2`D).
...
**Table `r plus('table.number.printed')`. Relationship of PIK3CA oncogene mutations with estimates of immune cell types across TCGA.**


```{r pik3catable, results = 'asis' }


##hit.kable2('PIK3CA.mut')

out = o.g.sig[ grepl('PIK3CA\\.', o.g.sig$Variant) & o.g.sig$P > 1.3 , ]
out = out[! grepl('\\.[gl]$', out$Variant), ]
out = data.frame(
    Cohort = out$Cohort,
    Signature = gsub('\\.sig$', '', out$Sig),
    Variant = gsub('.mut', ' mutant', out$Variant),
    Association = paste0(out$Effect, ' (', out$P, ')')
    )
out$Variant = gsub('.p.', ' ', out$Variant, fixed = TRUE)
...
**Table `r plus('table.number.printed')`. Association of FGFR3 mutations with estimates of immune cell types across TCGA.**


```{r fgfr3table, results='asis'}

  hit.kable2('FGFR3.mut', coords = FALSE)
...
(Fig `r foo = plus('figure.number'); foo`A and Table `r plus('table.number')`).
Death inducer-obliterator 1 (DIDO1) mutations were similarly associated with
higher NK cell estimates (Fig `r foo`B) in head/neck cancer.
...
```{r , fig.width = 7.5, fig.height = 3.75, fig.keep = 'last', fig.show = 'hide', dev='tiff', dev.args = list(compression = 'lzw'), dpi = 600}

##pdf(file = 'foo.pdf', width = 10, height = 5)
grid.arrange(
    mybox('CASP8.mut', 'TCD8.sig', 'Head_and_Neck_Cancer', rnamut) + annotation_custom(a_grob),
    mybox('DIDO1.mut', 'NK.sig', 'Head_and_Neck_Cancer', rnamut) + annotation_custom(b_grob),
    ncol = 2  )
##dev.off()
...
**Figure `r plus('figure.number.printed')`. Mutations in cell death pathways.**
...
**Table `r plus('table.number.printed')`. Association of mutations in cell death pathway genes CASP8 and DIDO1 with estimates of immune cell types across TCGA.**

```{r deathtable, results='asis'}

  hit.kable2(c('CASP8.mut', 'DIDO1.mut'), coords = FALSE )
...
lymphocyte abundance (Fig `r fig.landscape`). This common pattern of CNA loss leading to
...
TCD8 (CD8+ T cells): (`r paste(decon_signature_list$TCD8.sig, collapse = ", ")`) Source: Mining
...
Treg (Regulatory T Cells): (`r paste(decon_signature_list$Treg.sig, collapse = ", ")`)
...
Tcell (Pan T-Cell): (`r paste(decon_signature_list$Tcell.sig, collapse = ", ")`) Mining of
...
Bcell (B-cell): (`r paste(decon_signature_list$Bcell.sig, collapse = ", ")`) Source: Mining of
immune signatures in tumors using CD19 as sentinel
marker. Reciprocal-Mutual-Rank methods were used to identify
transcripts most intimately associated with sentinel markers.

Mono (Monocyte lineage): (`r paste(decon_signature_list$Mono.sig, collapse = ", ")`)
...
M2mf (M2 Macrophage):  (`r paste(decon_signature_list$MFm2.sig, collapse = ", ")`)
...
NK (Natural Killer cells): (`r paste(decon_signature_list$NK.sig, collapse = ", ")`)
...
```{r SFig-1, fig.width = 7.5, fig.height = 7.5, fig.keep = 'last', fig.show = 'hide', dev='tiff', dev.args = list(compression = 'lzw'), dpi = 600}

c9mix = o.g.rich [ o.g.rich$Chr == 9 & o.g.rich$Sig == 'TCD8.sig' &
                           grepl("\\.l$", o.g.rich$Variant) &
                           o.g.rich$Cohort %in% c('Pancreatic_Cancer', 'Melanoma', 'Head_and_Neck_Cancer') , ]

c9a = ddply(c9mix, .(Sig, cnv),  function(x) {
    y = x[1, , drop = FALSE]
    y$Cohort = 'Mean'
    y$P = mean(x$P, na.rm = TRUE)
    y$Effect = mean(x$Effect, na.rm = TRUE)
    y$rnacnvcor = mean(x$rnacnvcor, na.rm = TRUE)
    y$rnasigcor = mean(x$rnacnvcor, na.rm = TRUE)
    y
})

c9.merge = rbind.fill(c9mix, c9a)
c9.merge$Data = ifelse ( c9.merge$Cohort == 'Mean', 'Mean', 'Individual')

##pdf(file = 'foo.pdf')
p = manhat3(c9.merge, my.color = 'Cohort', variant = c('CDKN2A.l', 'JAK2.l'), vjust = -1.5, lSize = 3)
##p = p + facet_wrap(.(Data), scales = 'free_y', ncol = 1)
p = p + facet_wrap(.(Data), ncol = 1)
p = p + ylim(-1.6, -0.6) + xlim(0,40)
p
##dev.off()
